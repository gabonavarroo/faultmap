...
Exact binomial fallback (when b + c < 25): Under H0, b ~ Binomial(b+c, 0.5). Two-sided p-value via math.lgamma (same pattern as Fisher exact). This handles small-sample slices correctly.
...
test_mcnemar_chi2_a_wins — b=30, c=10: significant, A wins, uses chi2
test_mcnemar_chi2_no_difference — b=20, c=20: not significant, tie
test_mcnemar_exact_a_wins — b=8, c=1: significant, exact binomial, A wins
test_mcnemar_exact_no_difference — b=5, c=5: not significant, tie
test_mcnemar_no_discordant_pairs — b=0, c=0: p=1.0, test="none", winner="tie"
test_mcnemar_all_discordant_one_direction — b=15, c=0: highly significant
test_mcnemar_b_wins — b=2, c=12: B wins
test_mcnemar_threshold_exact_vs_chi2 — b+c=24 uses exact, b+c=25 uses chi2
test_bh_correction_comparison — known p-values, verify adjusted values
test_bh_comparison_empty — empty list returns empty
test_bh_comparison_monotonicity — adjusted p-values non-decreasing
...
test_compare_mode1_a_wins_globally — precomputed scores, A clearly better globally
test_compare_mode1_per_slice_winners — cluster 0: A wins, cluster 1: B wins, cluster 2: tie
test_compare_mode1_tie — equal scores, no significant slices
test_compare_mode2_detected — references trigger Mode 2 (patch ReferenceScorer)
test_compare_mode3_detected — neither scores nor references → Mode 3 (patch EntropyScorer)
test_compare_scores_and_references_warns — both provided, Mode 1 wins with UserWarning
test_compare_scores_a_without_b_raises — asymmetric scores → ConfigurationError
test_compare_mismatched_lengths_raises — validation errors
test_compare_report_to_dict_serializable — JSON serializable
test_compare_report_str — string formatting works
test_compare_report_summary — summary method
test_compare_no_discordant_pairs — both models identical on all prompts
tests/test_report.py — comparison report formatting
test_comparison_no_significant_slices — empty slices message
test_comparison_with_slices_contains_key_values — winner, advantage rate
test_comparison_global_shown — global p-value and winner displayed
test_comparison_plain_returns_nonempty — basic sanity
test_comparison_rich_returns_nonempty — basic sanity
test_comparison_import_error_fallback — rich missing falls back to plain
...
faultmap/models.py — Add SliceComparison and ComparisonReport dataclasses
faultmap/comparison/__init__.py — Create with exports
faultmap/comparison/statistics.py — McNemar's test + exact binomial + BH correction
tests/test_comparison/__init__.py + tests/test_comparison/test_statistics.py — Statistical unit tests
Verify: pytest tests/test_comparison/ -v passes

Phase 2: Validation + fixtures
faultmap/utils.py — Add validate_comparison_inputs()
tests/conftest.py — Add make_comparison_data() + comparison_data fixture
Phase 3: Orchestration
faultmap/analyzer.py — Add compare_models() + _compare_models_async()
tests/test_analyzer.py — Add TestCompareModels class
Verify: pytest tests/test_analyzer.py::TestCompareModels -v passes

Phase 4: Reporting
faultmap/report.py — Add format_comparison_report() with rich + plain text
tests/test_report.py — Add comparison report tests
Verify: pytest tests/test_report.py -v passes

Phase 5: Exports + docs
faultmap/__init__.py — Export new types
docs/dev/PLAN-08-comparison.md — Save this plan to follow PLAN-01..07 convention
README.md — New sections
CLAUDE.md — Update tables, API contract, architecture
docs/dev/ARCHITECTURE.md — Pipeline + module contracts
CHANGELOG.md — v0.4.0 entry
examples/example_model_comparison.py — Standalone comparison example
examples/example_mode1_custom_scores.py — Append commented "next step: compare" section
notebooks/tutorial.ipynb — New comparison section
